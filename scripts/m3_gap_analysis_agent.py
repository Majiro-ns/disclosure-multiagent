...
4. 有報テキストに明示的な記載がない場合は has_gap: true とする。
   ...
   ※「明示的な記載」とは: キーワードの完全一致は不要。意味的に対応する記述があれば充足とみなす。
5. 確信度（confidence）は "high"/"medium"/"low" のいずれかで答える。
   - high: テキストに要件を満たす記述が明確に存在する（has_gap: false が確実）
   - medium: 要件に関連する記述はあるが、必要な詳細（数値・期間・KPI等）が不足している
             または、テキストが[中略]を含み判定対象範囲が不完全な可能性がある
   - low: テキストが断片的・短すぎて判断材料が不足している、
          またはセクションが本項目と無関係と思われる
...
## 出力フォーマット（JSONのみ返すこと）
以下のJSON形式のみで出力してください。説明文・コメント・```コードブロック``` は不要です。
...
※ has_gap の値: true（記載なし/不十分）または false（記載あり/充足）
※ confidence の値: "high"（確実）/ "medium"（部分的な証拠あり）/ "low"（判断材料不足）"""
