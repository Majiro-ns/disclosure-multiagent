...
FORBIDDEN_PATTERNS: list[tuple[str, str]] = [
    ...
    (r"絶対に?", "断言表現「絶対」"),
    (r"業界(?:トップ(?:クラス)?|No\.?1|一位|最高水準)", "根拠のない業界比較（「業界トップクラス」「業界最高水準」等）"),
    (r"確実に", "断言表現「確実に」"),
...
FEW_SHOT_EXAMPLES: dict[str, dict[str, str]] = {
    ...
    "従業員給与等の決定に関する方針": {
        "松": (
            ...
            "2. 業績連動賞与: 会社業績・部門業績・個人評価の三層連動。上限は基本給の[上限率（例: 150）]%\n"
            "3. 賃上げ方針: [年度（例: 2024）]年度より[継続年数（例: 3）]年連続のベースアップを実施。物価上昇率との乖離を年次レビュー\n"
    ...
    "人材育成方針": {
        "松": (
            ...
            "高める仕組みを整備しています。\n\n"
            "主な取り組み:\n"
            "- リスキリング: オンライン学習プラットフォームを全従業員に開放（[開始年度（例: 2024）]年度〜）\n"
            "- 1人当たり教育投資: 年間[投資額（例: 12.5）]万円（前年比[増減率（例: 15）]%増）\n"
            "- 次世代リーダー育成: 管理職候補者向け特別研修（年[人数（例: 30）]名）"
        ),
        "竹": (
    ...
    # ── MD&A（経営方針・経営環境）few-shot examples ── (dis_b04_a7)
    "経営方針・経営環境": {
        "松": (
            "【経営方針・経営環境（[当連結会計年度（例: 2026年3月期）]）】\n\n"
            "当社グループは[中期経営計画名（例: 中期経営計画2024-2026）]に基づき、"
            "[主力事業（例: 製造業・情報サービス）]を中核とした持続的成長戦略を推進しています。\n\n"
            "【経営環境認識】\n"
            "- マクロ環境: [事業環境（例: 国内需要の底堅さ・海外市場の競争激化・原材料コスト上昇）]\n"
            "- 主要リスク: [主要リスク（例: 為替変動・サプライチェーン混乱・DX対応の遅れ）]\n\n"
            "【重点施策と業績目標】\n"
            "- ROE目標: [ROE目標値（例: 10）]%（[目標年度（例: 2027）]年度）\n"
            "- 事業ポートフォリオ最適化: [施策（例: 非中核事業の整理・新規領域への投資加速）]\n"
            "- 株主還元: 配当性向[配当性向（例: 40）]%以上を維持方針\n\n"
            "上記施策を通じ、持続的な企業価値向上と社会課題解決の両立を目指します。"
        ),
        "竹": (
            "当社グループは[中期経営計画名（例: 中期経営計画2024-2026）]のもと、"
            "[主力事業（例: 製造業）]を中核とした成長戦略を推進しています。\n\n"
            "外部環境として[事業環境（例: デジタル化進展・原材料コスト上昇）]を主要課題と認識し、"
            "事業モデルの変革と収益基盤の強化を重点施策としています。"
            "資本効率の向上（ROE目標[ROE目標値（例: 10）]%）と安定的な株主還元を重視しています。"
        ),
        "梅": (
            "当社グループは[主力事業（例: 製造業・情報サービス）]を中核として、"
            "国内外の事業環境の変化に対応しながら持続的な企業価値向上を目指しています。"
        ),
    },
    # ── 事業等のリスク few-shot examples ── (dis_b04_a7)
    "事業等のリスク": {
        "松": (
            "【主要リスクと対応策（[当連結会計年度（例: 2026年3月期）]）】\n\n"
            "当社グループは以下のリスクが事業・財務・株価に重大な影響を与える可能性があると認識しています。\n\n"
            "①[リスク名（例: 市場競争の激化）]\n"
            "  発現可能性: [高/中/低（例: 中）] ／ 影響度: [大/中/小（例: 大）]\n"
            "  内容: [リスク内容（例: 主要市場での競合他社の参入増加・価格競争の激化）]\n"
            "  対応策: [対応策（例: 製品差別化・顧客基盤の多様化・コスト構造改革）]\n\n"
            "②[リスク名（例: 法規制の変更）]\n"
            "  発現可能性: [高/中/低（例: 低）] ／ 影響度: [大/中/小（例: 中）]\n"
            "  内容: [リスク内容（例: 環境規制強化に伴うコスト増加・事業制限）]\n"
            "  対応策: [対応策（例: 法務・コンプライアンス体制の強化・規制動向のモニタリング）]\n\n"
            "上記リスクの管理状況はリスク管理委員会（[開催頻度（例: 四半期）]開催）で監督しています。"
        ),
        "竹": (
            "当社グループの事業運営に影響を与える主なリスクは以下の通りです。\n\n"
            "①[リスク名（例: 市場競争の激化）]: [リスク内容（例: 競合他社の参入による価格競争の激化）]。"
            "[対応策（例: 製品差別化と顧客基盤多様化）]を推進しています。\n\n"
            "②[リスク名（例: 為替変動リスク）]: [リスク内容（例: 外貨建て取引における収益への影響）]。"
            "[対応策（例: ヘッジ取引の活用・外貨建て費用との自然ヘッジ）]により影響を軽減しています。"
        ),
        "梅": (
            "当社グループの事業遂行上の主なリスクとして、"
            "[リスク①（例: 市場競争の激化）]、[リスク②（例: 為替変動）]、"
            "[リスク③（例: 法規制の変更）]等が挙げられます。"
            "各リスクへの対応は所管部門が行い、重要事項は経営会議に報告しています。"
        ),
    },
    # ── コーポレートガバナンスの状況 few-shot examples ── (dis_b04_a7)
    "コーポレートガバナンスの状況": {
        "松": (
            "【コーポレートガバナンス体制】\n\n"
            "当社は[機関設計（例: 監査等委員会設置会社）]を採用し、"
            "取締役会の監督機能強化と経営の透明性向上を図っています。\n\n"
            "【取締役会構成（[当期末日（例: 2026年3月31日）]現在）】\n"
            "- 総員数: [総員数（例: 9）]名（うち独立社外取締役[独立社外取締役数（例: 4）]名・比率[比率（例: 44）]%）\n"
            "- 女性取締役: [女性取締役数（例: 2）]名（比率[比率（例: 22）]%）\n"
            "- 外国籍取締役: [外国籍取締役数（例: 1）]名\n\n"
            "【実効性評価】\n"
            "取締役会の実効性評価を[実施頻度（例: 毎期）]実施し、[評価機関（例: 外部機関）]による"
            "客観的評価を取り入れています。[年度（例: 2026）]年度評価では"
            "[評価結果概要（例: 戦略的議論の充実・ダイバーシティ推進が課題として特定）]されました。"
        ),
        "竹": (
            "当社は[機関設計（例: 監査等委員会設置会社）]を採用し、取締役会の監督機能強化を図っています。\n\n"
            "取締役会は[総員数（例: 9）]名（うち独立社外取締役[独立社外取締役数（例: 4）]名・比率[比率（例: 44）]%）で構成され、"
            "多様な知見を活かした経営監督を実施しています。"
            "取締役会の実効性評価を[実施頻度（例: 毎期）]実施し、継続的な改善に努めています。"
        ),
        "梅": (
            "当社は[機関設計（例: 監査等委員会設置会社）]を採用しています。"
            "取締役会は[総員数（例: 9）]名（うち独立社外取締役[独立社外取締役数（例: 4）]名）で構成され、"
            "経営の透明性と監督機能の強化を図っています。"
        ),
    },
    # ── 重要な契約等（2025年3月期新規適用）few-shot examples ── (dis_b04_a7)
    "重要な契約等": {
        "松": (
            "【重要な契約等】\n\n"
            "①[契約種別（例: 業務提携契約）]\n"
            "  契約当事者: 当社 ／ [相手方（例: [相手方社名（例: A社）]）]\n"
            "  契約期間: [契約開始日（例: 2025年4月1日）]〜[契約終了日（例: 2028年3月31日）]\n"
            "  契約の概要: [契約概要（例: [主力製品]の共同開発・生産委託に関する業務提携）]\n"
            "  財務的影響: [財務影響（例: 年間売上高の約XX%相当の取引規模）]\n\n"
            "②[契約種別（例: 重要な借入契約）]\n"
            "  契約当事者: 当社 ／ [相手方（例: 主要取引金融機関）]\n"
            "  借入限度額: [借入額（例: 100）]億円（コミットメントライン）\n"
            "  契約期間: [契約開始日（例: 2025年4月1日）]〜[契約終了日（例: 2027年3月31日）]\n"
            "  財務上の制約条項: [制約条項（例: 純資産維持・インタレストカバレッジ等の財務制限条項あり）]"
        ),
        "竹": (
            "当連結会計年度において締結した重要な契約は以下の通りです。\n\n"
            "①[契約種別（例: 業務提携契約）]: [相手方（例: A社）]との[契約内容（例: 共同開発・生産委託に関する業務提携）]。"
            "契約期間: [契約開始日（例: 2025年4月）]〜[契約終了日（例: 2028年3月）]。\n\n"
            "②[契約種別（例: 重要な借入契約）]: [相手方（例: 主要取引金融機関）]と"
            "[借入額（例: 100）]億円のコミットメントライン契約を締結。"
            "期間: [契約開始日（例: 2025年4月）]〜[契約終了日（例: 2027年3月）]。"
        ),
        "梅": (
            "当連結会計年度において[件数（例: 2）]件の重要な契約を締結しました。\n"
            "主なものとして、[相手方（例: A社）]との[契約種別（例: 業務提携契約）]（"
            "[契約期間（例: 2025年4月〜2028年3月）]）があります。"
        ),
    },
    "気候変動に関するガバナンス体制の開示": {
        "松": (
            ...
            "サステナビリティ委員会（[開催頻度（例: 四半期）]開催）を通じて定期的な報告・監督を実施しています。\n\n"
            "経営陣の役割:\n"
            "- [役職名（例: CSO・サステナビリティ担当役員）]（委員長）が気候変動戦略の立案・執行を統括\n"
            "- 気候関連リスク・機会は[部門名（例: サステナビリティ推進部）]が一次管理し、[報告頻度（例: 四半期）]で経営会議に報告\n\n"
            "スキル・能力開発:\n"
            "- 取締役[取締役人数（例: 3）]名が気候変動・サステナビリティ関連の専門研修を修了\n"
            "- 役員報酬の[比率（例: 10）]%を気候変動目標達成度に連動"
        ),
        "竹": (
            "取締役会は気候変動をグループの重要課題と位置付け、"
            "サステナビリティ委員会（[開催頻度（例: 四半期）]開催）を通じて気候関連リスク・機会の監督を行っています。"
            "[役職名（例: CSO）]が気候変動戦略の執行責任を担い、重要な事項は取締役会に報告されます。"
        ),
        "梅": (
            "取締役会はサステナビリティ委員会を通じて気候変動リスク・機会を監督しています。"
            "[役職名（例: CSO）]が気候変動対応の執行責任者です。"
        ),
    },
}
...
SECTION_NORMALIZE: dict[str, str] = {
    ...
    "人材育成方針": "人材育成方針",
    "人材育成": "人材育成方針",
    # MD&A・事業等のリスク・ガバナンス・重要な契約等（dis_b04_a7）
    "経営方針・経営環境": "経営方針・経営環境",
    "MD&A": "経営方針・経営環境",
    "経営環境": "経営方針・経営環境",
    "経営方針": "経営方針・経営環境",
    ...
    "主要リスク": "事業等のリスク",
    "リスク情報": "事業等のリスク",
    "コーポレートガバナンスの状況": "コーポレートガバナンスの状況",
    "コーポレートガバナンス": "コーポレートガバナンスの状況",
    "ガバナンス体制": "コーポレートガバナンスの状況",
    "取締役会構成": "コーポレートガバナンスの状況",
    "重要な契約等": "重要な契約等",
    "重要契約": "重要な契約等",
    "重要な契約": "重要な契約等",
    # SSBJ関連マッピング
...
## 開示レベルの定義
- 松（充実）: KPI・数値目標・ガバナンス体制・第三者保証の言及を含む充実した記載（200〜480字、目標300字）
- 竹（標準）: 必須項目を過不足なく満たす実務的な記載（100〜260字、目標150字）
- 梅（簡潔）: 法令義務の最小限を満たす簡潔な記載（50〜120字、目標80字）
...
4. 禁止事項:
   ...
   - 保証できない断言表現（「必ず」「絶対に」等）
   - 根拠のない業界比較（「業界トップ水準」等）
5. 文字数を厳守してください（範囲外は品質チェックで再生成が必要になります）:
   - 梅: 50〜120字（目標80字）
   - 竹: 100〜260字（目標150字）
   - 松: 200〜480字（目標300字）"""
...
def check_char_count(text: str, level: str) -> tuple[bool, str, int]:
    ...
    if char_count > limits["max"]:
        return False, f"文字数超過: {char_count}字（{level}の最大{limits['max']}字を超過 ― 目標{limits['target']}字に短縮してください）", char_count
