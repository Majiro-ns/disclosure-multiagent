...
- **拡張プロファイル対応（プラグイン設計）** — 大手監査法人が重視する開示観点に対応した YAML プロファイルを追加可能（デフォルト非同梱）
...
├── profiles/                # 拡張開示チェックプロファイル（非同梱・プラグイン設計）
│   ├── deloitte/
│   ├── kpmg/
│   ├── pwc/
│   └── ey/
...
本ソフトウェアは情報提供を目的としており、法的助言を構成するものではありません。
会計・税務・法律に関する判断は、必ず有資格の専門家にご相談ください。

This software is provided for informational purposes only and does not constitute
legal, accounting, or tax advice. Please consult qualified professionals for
decisions related to accounting, tax, or legal matters.
