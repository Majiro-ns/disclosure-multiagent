...
[前回の記事（Phase1）](https://zenn.dev)では **disclosure-multiagent** の M1〜M5 パイプラインを紹介した。有報PDFを入力すると、法令改正に基づくギャップを検出し、松竹梅3水準の改善提案Markdownを自動生成するシステムだ。累計735 passed, 3 skipped で品質を担保している。
...
| テストファイル | Phase | 件数 | 内容 |
|---|---|---|---|
| test_m1_pdf_agent.py | 1 | 47件 | PDF解析・セクション分割 |
| test_m2_law_agent.py | 1 | 26件 | 法令YAML読み込み |
| test_m3_gap_analysis.py | 1 | 30件 | ギャップ分析（モック） |
| test_m4_proposal.py | 1 | 55件 | 松竹梅提案文生成 |
| test_m5_report.py | 1 | 46件 | Markdownレポート統合 |
| test_e2e_pipeline.py | 1 | 22件 | E2Eパイプライン |
| test_m6_law_url_collector.py | 2 | 13件 | e-Gov APIマッチング |
| test_m7_edinet_client.py | 2 | 22件 | EDINETクライアント |
...
disclosure-multiagentは当初「BSLライセンス+コンサル」戦略（B戦略）で検討していたが、開発方針として「記事マーケティング+受託」戦略（C戦略）に転換した。
...
USE_MOCK_LLM=false python3 scripts/run_e2e.py \
    "scripts/tests/samples/company_a.pdf" \
    --company-name "サンプル社A" \
    --fiscal-year 2025 \
    --level 竹
```
...
| Phase | テスト数（累計） | 主要機能 |
|---|---|---|
| Phase1 | 226件 | M1-M5: PDF解析→法令照合→提案生成→Markdown出力 |
| Phase2 | 735 passed, 3 skipped | M6: 法令URL自動収集 / M7: EDINET自動取得 / M8: 多年度比較 / M9: Word/Excel出力 |
...
- [disclosure-multiagent GitHub（リポジトリ）](https://github.com/Majiro-ns/disclosure-multiagent)
