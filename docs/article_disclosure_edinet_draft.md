...
EDINET から有報を手動ダウンロードして、毎回 Excel に貼り付けて法令チェック——。
...
```python
from m1_pdf_agent import extract_report, StructuredReport

# EDINET でダウンロードした PDF をそのまま渡せる
pdf_path = download_pdf("S100VHUZ", output_dir="./downloads")
report: StructuredReport = extract_report(pdf_path)
...
# 2024年度・3月決算（2024/04/01 〜 2025/03/31 が参照期間）
# ※ FY2024 は SSBJ 2025 基準が既に収録されているため SSBJ 25 件が適用対象に含まれる
...
# → 適用法令数: 30  （人的資本5件 + SSBJ 25件）
```

`laws/` 配下に現在 4 つの YAML が存在します（適用件数は分析する事業年度により変動）。

- `laws/human_capital_2024.yaml`: 人的資本開示（男女賃金格差・育休取得率等）8 件
- `laws/ssbj_2025.yaml`: SSBJ 気候変動開示基準 25 件（GHG・移行計画・削減目標等）
- `laws/shareholder_notice_2025.yaml`: 株主総会招集通知・ガバナンス 16 件
- `laws/banking_2025.yaml`: 銀行業固有開示基準
...
| ID | 項目 | 必須 |
|----|------|-----|
| CL-026 | GHG 排出量（Scope1・2）開示 | 必須 |
| CL-027 | GHG 排出量（Scope3）開示 | △（大規模企業は実質必須）|
| CL-028 | 気候変動ガバナンス体制 | 必須 |
| CL-029 | 気候変動リスク・機会の特定と財務影響 | 必須 |
| CL-030 | 気候変動移行計画 | 必須 |
| CL-031 | シナリオ分析 | △（大規模企業は必須）|
| CL-032 | GHG 削減目標・進捗 | 必須 |
...
```bash
# リポジトリ取得
git clone https://github.com/Majiro-ns/disclosure-multiagent
...
**② モック設計で開発コストを最小化**
`USE_MOCK_EDINET=true` / `USE_MOCK_LLM=true` で EDINET API キーも Claude API キーも不要でローカル開発・テストが完結します。CI/CD でも全テストがパスします（プロジェクト全体 408 テスト ALL PASS 確認済み）。
...
本記事で紹介した実装は **disclosure-multiagent** として GitHub で公開しています。
