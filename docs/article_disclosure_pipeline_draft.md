...
有報チェックを「1つのPythonスクリプト」で実装した場合の問題点を考えてみる。
...
各エージェントは**データクラスをインターフェース**として持ち、他エージェントの内部実装に依存しない。M1が「どうやってPDFを開くか」をM3は知らなくていい。
...
この設計は「M3が核になる」という構造的判断だ。ギャップ分析が全データの交差点になるため、データクラス定義の場所として最も自然だ。
...
M3は本システムの**中核エージェント**だ。M1の有報テキストとM2の法令リストを突き合わせ、LLMが「記載されているか / ギャップがあるか」を判定する。
...
**ハルシネーション対策が重要**だ。制約を明示しないと、LLMは「一般的なベストプラクティス」で指摘を増やし、false positiveが増加する。
...
SSBJ_KEYWORDS = [
    "SSBJ", "気候変動", "GHG", "Scope1", "移行計画", "物理的リスク", ...
]
BANKING_KEYWORDS = [
...
M4はM3が検出した「ギャップ（不足開示項目）」に対して、**3水準（松竹梅）の改善文案**を生成する。
...
コンプライアンス担当者が**最も重要な指摘から確認できる**よう、「追加必須 → 修正推奨 → 参考」の順で自動ソートする。
...
```bash
# M1単体テスト（PDFなし・高速）
python3 -m pytest tests/test_m1_pdf_agent.py -v
# → 23 passed in 0.4s

# M3テスト（USE_MOCK_LLM=true）
USE_MOCK_LLM=true python3 -m pytest tests/test_m3_gap_analysis_agent.py -v
# → 31 passed in 1.2s

# 全テスト（318テスト以上・モードモード）
USE_MOCK_LLM=true python3 -m pytest tests/ -v
# → 318 passed in 8.7s
```
...
Streamlit UIでは、PDFをアップロードするだけで「デモモード（`pipeline_mock()`）」が動作する。APIキーなしでも動作確認できるのはハードルを下げる重要な設計だ。
...
「どのエージェントまで使うか」をビジネスモデルに直結させられるのは、**エージェントが明確に分離されているから**だ。
...
**単一責務 × インターフェース契約 × モック設計**の3原則が、318件以上のテストを持つ保守可能なコードベースを実現した。
