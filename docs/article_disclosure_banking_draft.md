...
本記事では、Python製マルチエージェントシステム **disclosure-multiagent** を使って、銀行業の有報開示チェックを自動化する業種特化モジュールの実装を解説する。`laws/banking_2025.yaml` に定義した銀行業特化チェック項目（10件）を M2法令収集エージェント（`m2_law_agent.py`）が読み込み、M3ギャップ分析エージェントが `BANKING_KEYWORDS` で有報の関連セクションを自動検出する仕組みだ。
...
**① バーゼルIII第3の柱（市場規律）**
金融庁告示・銀行法施行規則に基づき、自己資本比率・流動性・レバレッジ比率の開示が義務化されている。プルーデンシャル規制の透明化が目的だ。

**② 金融再生法（1998年制定）**
不良債権の実態を社会に示すための法律。与信先を破綻先・実質破綻先・破綻懸念先・要管理先の4区分で分類し、残高・保全状況を開示する義務がある。
...
RWA（bk-2025-002）は、信用リスク/市場リスク/オペレーショナルリスクの内訳と算出方法・前期比較・変動要因説明を要する。内部格付け手法採用行はOVA・OV1テンプレートへの対応も必要で、開示負担が大きい項目だ。
...
貸倒引当金（bk-2025-007）は、個別貸倒引当金（要管理先以下の個別与信先）と一般貸倒引当金（正常先・要注意先の損失実績率ベース）の区分で、計上額・算出方法・引当率の根拠を記載する。IFRS9採用行は予想信用損失（ECL）3ステージ方式の概要説明も必要だ。
...
bk-2025-009のIRRBBは、バーゼルIIIのIRRBB基準（2016年）の国内実施（2020年3月末施行）に対応する。金利変動シナリオとしてパラレルシフト・スティープニング・フラットニング等を用い、EVE（銀行全体の経済価値への影響）とNII（1年間の純金利収益への影響）をそれぞれ開示する。
...
`load_law_context()` の内部でこのリストを走査し、一致するエントリが0件のカテゴリを検出した場合は `warnings` にアラートを追加する処理が走る。
...
3カテゴリのキーワードを統合した `ALL_RELEVANCE_KEYWORDS` が、有報の各セクションがチェック対象の関連セクションかどうかを判定するために使われる。
...
例えば有報のリスク管理の状況 > 信用リスク管理セクションは、見出しに `信用リスク` が含まれるため `BANKING_KEYWORDS` にマッチし、チェック対象として抽出される。

逆に連結財務諸表注記 > 固定資産の減価償却方法のようなセクションはいずれのキーワードにもマッチしないため、スキップされる。
...
内部のコンプライアンス・IR部門が有報審査を行うが、バーゼルIII開示の複雑さ（OVA/OV1テンプレート・IRRBB開示等）により専門ツールへのニーズがある。提供価値はギャップ検出の自動化よりも開示の充実度スコアリング（松竹梅）だ。
...
disclosure-multiagent の設計は、法令の追加を YAML 追加のみで完結させるというオープンクローズド原則を貫いている。銀行業有報の複雑な開示要件も、この設計によって低コストで自動チェック体制を構築できる。

---

### ソースコード

本記事で解説した実装は **[disclosure-multiagent](https://github.com/Majiro-ns/disclosure-multiagent)** として GitHub で公開している。

- 銀行業特化法令YAML: `laws/banking_2025.yaml`
- M2法令収集エージェント（マルチYAML対応）: `scripts/m2_law_agent.py`
- M3ギャップ分析（BANKING_KEYWORDS）: `scripts/m3_gap_analysis_agent.py`
- M4松竹梅提案エージェント: `scripts/m4_proposal_agent.py`

---

## 参考資料

- [バーゼルIII 第3の柱（金融庁告示・銀行法施行規則）](https://www.fsa.go.jp/policy/ginkou/basel.html)
- [金融再生法 第6〜8条（不良債権4区分）](https://elaws.e-gov.go.jp/document?lawid=410AC0000000132)
- [主要行等向けの総合的な監督指針（金融庁）](https://www.fsa.go.jp/news/r6/ginkou/20250324.html)
- [バーゼルIII IRRBB基準（BIS 2016年）](https://www.bis.org/bcbs/publ/d368.htm)
- [GHGプロトコル コーポレート基準](https://ghgprotocol.org/corporate-standard)
